function [h_filtered,omeg,M, wc,Delta_pr, Delta_sr] = calculate_filter_deviations_f4(h, wp, ws, delta_pr_max, delta_sr_max,win_t)
    M = length(h) - 1; % Initial order
    wc = sqrt(wp * ws); % Initial cutoff frequency
    
    while true
        
        [h_filtered,omeg] = create_filter(M, wc, win_t);
...
function [h_filtered, omeg] = create_filter(M, wc, window_t)
    [h_filtered,omeg] = fir1(M-1, wc/pi, window_t(M));
end
